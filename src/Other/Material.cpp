...
Texture::Texture(const Sprite* sprite) : _surface(sprite->surface()), _texture(sprite->defaultTexture())
{
	_size = {_surface->w, _surface->h};
}
Texture::Texture(const Font* font, const std::string& text, const Color& color)
{
