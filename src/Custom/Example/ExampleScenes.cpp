...
namespace examples
{
	...
	void ExampleScenes::textAndTweensScene()
	{
		...
		Tweener::delayedCall([text] { text->setColor(text->getColor().withAlpha(1)); }, 1);
		Tweener::floatTo(0, 1, 3, [text1](float value) { text1->setColor(Color::RED.withAlpha(value)); });
		Tweener::colorTo(Color::WHITE.withAlpha(0), Color::BLUE, 3, [text2](Color value) { text2->setColor(value); });
		auto tween = Tweener::vec2To(text2->transform()->pos() - glm::vec2(1, 0), text2->transform()->pos() + glm::vec2(1, 0), 3,
