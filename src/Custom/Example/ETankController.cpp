...
namespace examples
{
	ETankController::ETankController(GameObject* obj, float moveSpeed, float rotationSpeed): Component(obj), moveSpeed(moveSpeed), rotationSpeed(rotationSpeed)
	{
		rb = obj->getComponent<Rigidbody>();
	}

	void ETankController::fixedUpdate()
	{
		float moveDir = 0;
