...
void PositionSolver::solveCollisions(const std::vector<Collision>& collisions)
{
	for (auto& collision : collisions)
	{
		...
		rb1->moveTo(rb1->transform()->pos() - resolutionVector * rb1->_invMass);
		rb2->moveTo(rb2->transform()->pos() + resolutionVector * rb2->_invMass);
	}
}
...
void ImpulseSolver::solveCollisions(const std::vector<Collision>& collisions)
{
	for (auto& collision : collisions)
	{
		...
		auto r1 = contactPoint - rb1->transform()->pos();
		auto r2 = contactPoint - rb2->transform()->pos();
